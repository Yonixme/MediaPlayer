...
class MusicServiceManagerImpl @Inject constructor(
    ...
    private val _currentSong: MutableStateFlow<CurrentSongState> = MutableStateFlow(CurrentSongState.Loading)
    val currentSongNew: Flow<CurrentSongState> = _currentSong

    private var musicService: MusicServiceNew? = null
    private var isBound = false

    private val serviceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            val binder = service as MusicServiceNew.GetServiceBinder
    ...
    override fun bindService(){
        Intent(context, MusicServiceNew::class.java).also { intent ->
    ...
    override fun onPlay(uri: String) {
        startServiceWithCommand(
            action = MusicServiceNew.COMMAND_ON_PLAY_MUSIC,
            uri = uri
        )
    }
    ...
    override fun onPause(uri: String) {
        startServiceWithCommand(
            action = MusicServiceNew.COMMAND_ON_PAUSE_MUSIC,
            uri = uri
        )
    }
    ...
    override fun onStop(uri: String) {
        startServiceWithCommand(
            action = MusicServiceNew.COMMAND_ON_STOP_MUSIC,
            uri = uri
        )
    }
    ...
    override fun pauseMusic(uri: String) {
        startServiceWithCommand(
            action = MusicServiceNew.COMMAND_PAUSE_PLAYING,
            uri = uri
        )
    }
    ...
    override fun continueMusic(uri: String) {
        startServiceWithCommand(
            action = MusicServiceNew.COMMAND_CONTINUE_PLAYING,
            uri = uri
        )
    }
    ...
    override fun nextSong(uri: String) {
        startServiceWithCommand(
            action = MusicServiceNew.COMMAND_NEXT_SONG,
            uri = uri
        )
    }
    ...
    override fun previousSong(uri: String) {
        startServiceWithCommand(
            action = MusicServiceNew.COMMAND_PREVIOUS_SONG,
            uri = uri
        )
    }
    ...
    private fun startServiceWithCommand(action: String, uri: String){
        ...
        val intent = Intent(context, MusicServiceNew::class.java)
        intent.action = action
        intent.putExtra(MusicServiceNew.ID_URI, uri)
